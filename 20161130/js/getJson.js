$(function() {
  $.getJSON("data.json", function(data) {
    ...
    for (var i = 0; i < len; i++) {
      ...
      ulObj1.append($('<a class="grid__item ' + data[i].Category + '" href="#" data-category="' + data[i].Category + '"><h2 class="title title==preview">' + data[i].Headline + '</h2><div class="loader"></div><span class="category">' + data[i].Category + '</span><div class="meta meta==preview">' + firstBodyPreview + '<span class="meta__date"><i class="fa fa-calendar-o"></i>' + document.lastModified + '</span><span class="meta__reading-time"><i class="fa fa-clock-o"></i>' + bodyLength + '文字' + '</span></div></a>'));
    ...
    for (var i = 0; i < len; i++) {
      ...
      ulObj4.append($('<p><a href="#seq5">' + HeadlinePreview + '</a></p>'));
    }
  });
});